...
export function parseApiError(error: any): NeopleApiError {
  ...
  const status = error.status || error.response?.status || HttpStatusCode.INTERNAL_SERVER_ERROR;
  const code = error.code || error.response?.data?.error?.code || 'SYSTEM_ERROR';
  const message = error.message || error.response?.data?.error?.message || ERROR_MESSAGES[code] || '알 수 없는 오류';
...
export function isMaintenanceError(error: any): boolean {
  return isErrorCode(error, 'DNF980' as ApiErrorCode) || isErrorCode(error, 'CY980' as ApiErrorCode);
}
...
export function isRateLimitError(error: any): boolean {
  const apiError = parseApiError(error);
  return apiError.status === HttpStatusCode.TOO_MANY_REQUESTS || 
